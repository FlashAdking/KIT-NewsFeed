...
export class AdminService {
  ...
  // Check if admin can promote to specific level
  static canPromoteToLevel(admin: IUser, targetLevel: string): boolean {
    if (!admin.adminProfile) return false;
  ...
  static async getPendingFacultyApprovals(adminId: string) {
    try {
      ...
      // Check if admin has permission to approve faculty
      if (!admin.adminProfile?.canManageClubs && admin.adminProfile?.adminLevel !== 'super') {
        throw new Error('Insufficient permissions to manage faculty approvals');
      }
      
      const pendingFaculty = await User.find({
        role: 'faculty',
        'facultyProfile.isApproved': false,
        isActive: true // Only show active pending accounts
      })
      .select('-password') // Include faculty profile explicitly
      ...
      throw new Error(error.message || 'Failed to fetch pending faculty');
    }
  }

  // Approve/reject faculty account
  static async processFacultyApproval(
    adminId: string,
    facultyId: string,
    ...
    try {
      ...
      if (!admin.adminProfile?.canManageClubs && admin.adminProfile?.adminLevel !== 'super') {
        throw new Error('Insufficient permissions to approve faculty');
      }
      ...
      if (faculty.facultyProfile?.isApproved === true) {
        throw new Error('Faculty account is already approved');
      }
      ...
      if (decision === 'approved') {
        ...
        faculty.facultyProfile.isApproved = true;
        faculty.facultyProfile.approvedBy = new Types.ObjectId(adminId);
        faculty.facultyProfile.approvedAt = new Date();
        faculty.isVerified = true; // Also mark as verified

        console.log(`✅ Faculty approved: ${faculty.email} by admin: ${admin.email}`);
      } else {
        // For rejection, deactivate the account and add rejection info
        ...
        faculty.facultyProfile.rejectedBy = new Types.ObjectId(adminId);
        faculty.facultyProfile.rejectedAt = new Date();
        faculty.facultyProfile.rejectionNotes = notes;

        console.log(`❌ Faculty rejected: ${faculty.email} by admin: ${admin.email}`);
      }
      
      await faculty.save();
      
      // Log the approval/rejection for audit trail
      console.log(`Faculty ${decision}: ${faculty.fullName} (${faculty.email}) by ${admin.fullName}`);
      
      return {
        message: `Faculty account ${decision} successfully`,
        faculty: {
          _id: faculty._id,
      ...
      throw new Error(error.message || 'Failed to process faculty approval');
    }
  }

  // Get faculty approval history (for admin dashboard)
  static async getFacultyApprovalHistory(adminId: string, limit: number = 50) {
    try {
      const admin = await User.findById(adminId);
      if (!admin || admin.role !== 'admin') {
        throw new Error('Admin access required');
      ...
      const approvedFaculty = await User.find({
        role: 'faculty',
        $or: [
          { 'facultyProfile.isApproved': true },
          { isActive: false, 'facultyProfile.rejectedBy': { $exists: true } }
        ]
      })
      .select('-password')
      .populate('facultyProfile.approvedBy', 'fullName email')
      .populate('facultyProfile.rejectedBy', 'fullName email')
      .sort({ 'facultyProfile.approvedAt': -1, 'facultyProfile.rejectedAt': -1 })
      .limit(limit);
  ...
  // ✅ NEW: Get pending club representative requests specifically
  ...
  // ✅ NEW: Process club representative requests specifically
  ...
  // ✅ NEW: Get club representative history
  ...
  private static canGrantPermissions(admin: IUser, permissions: string[]): boolean {
    ...
    if (admin.adminProfile.adminLevel === 'super') return true;
