...
export class AdminService {
  ...
  static async promoteToAdmin(
    ...
    try {
      ...
      if (!promoter || promoter.adminProfile?.adminLevel !== 'super') {
      ...
      return {
        message: 'User promoted to Post Moderator successfully',
        user: targetUser
      };
    } catch (error: any) {
  ...
  // In AdminService.ts
  static async revokeAdmin(promoterId: string, targetUserId: string) {
    const promoter = await User.findById(promoterId);
    if (!promoter || promoter.adminProfile?.adminLevel !== 'super') {
      throw new Error('Only Super Admin can revoke admin access');
    }
    ...
    if (targetUser.adminProfile?.adminLevel === 'super') {
      throw new Error('Cannot revoke Super Admin access');
    }

    targetUser.role = 'student';
    targetUser.adminProfile = undefined;
    await targetUser.save();
